...
func TestAccountListEmpty(t *testing.T) {
	dsp := runGeth(t, "account", "list")
	dsp.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	dsp := runGeth(t, "account", "list", "--datadir", datadir)
	defer dsp.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	dsp := runGeth(t, "account", "new", "--lightkdf")
	defer dsp.ExpectExit()
	dsp.Expect(`
...
`)
	dsp.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	dsp := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer dsp.ExpectExit()
	dsp.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(dsp.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(dsp.StderrText(), m) {
...
// https://github.com/dsplinz2019/dsplinz/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	dsp.SetTemplateFunc("keypath", func(file string) string {
