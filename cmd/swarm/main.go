...
import (
	...
	"github.com/dsplinz2019/dsplinz/swarm"
	bzzapi "github.com/dsplinz2019/dsplinz/swarm/api"
	swarmmetrics "github.com/dsplinz2019/dsplinz/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the Dsplinz API provider to use to settle SWAP payments",
	...
	DeprecatedRlzAPIFlag = cli.StringFlag{
		Name:  "ethapi",
...
// This init function sets defaults so cmd/swarm can run alongside dsp.
func init() {
...
var app = utils.NewApp(gitCommit, "Dsplinz Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-dsplinz Authors"
...
func bzzd(ctx *cli.Context) error {
	...
	//dsp only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for dsp and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the dsplinz node
	...
	//register BZZ as node.Service in the dsplinz node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	//register within the dsplinz node
