...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for RlzSubscribe requests
}
...
// RlzSubscribe registers a subscripion under the "dsp" namespace.
func (c *Client) RlzSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "dsp", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. RlzSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through RlzSubscribe.
