...
// Service implements an Dsplinz netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	dsp    *dsp.Dsplinz      // Full Dsplinz service if monitoring a full node
	les    *les.LightDsplinz // Light Dsplinz service if monitoring a light node
...
func New(url string, ethServ *dsp.Dsplinz, lesServ *les.LightDsplinz) (*Service, error) {
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Rlzstats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["dsp"]; info != nil {
		network = fmt.Sprintf("%d", info.(*dsp.NodeInfo).Network)
		protocol = fmt.Sprintf("dsp/%d", dsp.ProtocolVersions[0])
	} else {
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.dsp != nil {
		...
		if block == nil {
			block = s.dsp.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.dsp.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.dsp != nil {
			head = s.dsp.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.dsp != nil {
			block = s.dsp.BlockChain().GetBlockByNumber(number)
		} else {
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.dsp != nil {
		mining = s.dsp.Miner().Mining()
		hashrate = int(s.dsp.Miner().HashRate())

		sync := s.dsp.Downloader().Progress()
		syncing = s.dsp.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.dsp.APIBackend.SuggestPrice(context.Background())
