...
import (
	...
	"github.com/dsplinz2019/dsplinz/common/bitutil"
	"github.com/dsplinz2019/dsplinz/light"
)

const (
	// bloomServiceThreads is the number of goroutines used globally by an Dsplinz
...
func (dsp *LightDsplinz) startBloomHandlers() {
		...
		go func() {
			for {
				...
				case request := <-dsp.bloomRequests:
					...
					compVectors, err := light.GetBloomBits(task.Context, dsp.odr, task.Bit, task.Sections)
					if err == nil {
