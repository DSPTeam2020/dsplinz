...
import (
	...
	"github.com/dsplinz2019/dsplinz/les/flowcontrol"
...
func NewLesServer(dsp *dsp.Dsplinz, config *dsp.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(dsp.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, dsp.EventMux(), dsp.Engine(), newPeerSet(), dsp.BlockChain(), dsp.TxPool(), dsp.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(dsp.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(dsp.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(dsp.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(dsp.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(dsp.ChainDb())
	return srv, nil
}
