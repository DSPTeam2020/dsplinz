...
import (
	...
	"github.com/dsplinz2019/dsplinz/internal/jsre"
	"github.com/dsplinz2019/dsplinz/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Dsplinz requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var dsp = web3.dsp; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = dsp.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines dsp.getBalance(dsp.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Rlz instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Rlz metadata
	fmt.Fprintf(c.printer, "Welcome to the Rlz JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + dsp.coinbase);
		console.log("at block: " + dsp.blockNumber + " (" + new Date(1000 * dsp.getBlock(dsp.blockNumber).timestamp) + ")");
