...
func newTester(t *testing.T, confOverride func(*dsp.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Dsplinz service within
	...
	ethConf := &dsp.Config{
		...
		Rlzerbase: common.HexToAddress(testAddress),
		Rlzash: ethash.Config{
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return dsp.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register Dsplinz protocol: %v", err)
	}
