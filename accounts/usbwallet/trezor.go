...
import (
	...
	"github.com/dsplinz2019/dsplinz/accounts/usbwallet/internal/trezor"
...
// or whether the Dsplinz app was not started on it.
...
// Dsplinz address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.DsplinzAddress)
	if _, err := w.trezorExchange(&trezor.DsplinzGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.DsplinzTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.DsplinzTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
