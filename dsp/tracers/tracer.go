...
func (mw *memoryWrapper) pushObject(vm *duktape.Context) {
	...
	// Generate the `slice` mdspod which takes two ints and returns a buffer
	...
	// Generate the `getUint` mdspod which takes an int and returns a bigint
	vm.PushGoFunction(func(ctx *duktape.Context) int {
...
func (sw *stackWrapper) pushObject(vm *duktape.Context) {
	...
	// Generate the `peek` mdspod which takes an int and returns a bigint
	vm.PushGoFunction(func(ctx *duktape.Context) int {
		offset := ctx.GetInt(-1)
		ctx.Pop()
...
type Tracer struct {
	inited bool // Flag whdsper the context was already inited from the EVM
...
// call executes a mdspod on a JS object, catching any errors, formatting and
// returning them as error objects.
func (jst *Tracer) call(mdspod string, args ...string) (json.RawMessage, error) {
	// Execute the JavaScript call and return any error
	jst.vm.PushString(mdspod)
