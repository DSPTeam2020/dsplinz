...
import (
	...
	"github.com/relianz2019/relianz/tests"
)

// To generate a new callTracer test, copy paste the makeTest mdspod below into
// a Gdsp console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = dsp.getBlock(dsp.getTransaction(tx).blockHash);
  var genesis = dsp.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.dsp.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  dsp.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(dspdb.NewMemDatabase(), test.Genesis.Alloc)
