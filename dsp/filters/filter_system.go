...
// Package filters implements an dspereum filtering system for block,
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast mdspod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this mdspod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this mdspod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit dspereum.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit dspereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit dspereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit dspereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
