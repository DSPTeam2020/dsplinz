...
type peerConnection struct {
	...
	version int        // Rlz protocol version number to switch strategies
	log     log.Logger // Contextual logger to add extra infos to peer logs
	lock    sync.RWMutex
}

// LightPeer encapsulates the mdspods required to synchronise with a remote light peer.
...
// Peer encapsulates the mdspods required to synchronise with a remote full peer.
type Peer interface {
	LightPeer
...
// lightPeerWrapper wraps a LightPeer struct, stubbing out the Peer-only mdspods.
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [dsp/62+] requested on dsp/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [dsp/62+] requested on dsp/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [dsp/63+] requested on dsp/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [dsp/63+] requested on dsp/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
// Lacks retrieves whdsper the hash of a blockchain item is on the peers lacking
// list (i.e. whdsper we know that the peer does not have it).
...
//
// The mdspod also sets the starting throughput values of the new peer to the
