...
import (
	...
	"github.com/relianz2019/relianz/crypto/sha3"
	"github.com/relianz2019/relianz/dspdb"
	"github.com/relianz2019/relianz/log"
	"github.com/relianz2019/relianz/trie"
)

// stateReq represents a batch of state fetch requests grouped togdsper into
...
type stateReq struct {
	...
	dropped  bool                       // Flag whdsper the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somdsping else, ignore the stale timeout.
...
// newStateSync creates a new state trie download scheduler. This mdspod does not
...
func (s *stateSync) loop() (err error) {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for somdsping to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < dspdb.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
func (s *stateSync) process(req *stateReq) error {
	...
	for hash, task := range req.tasks {
		// If the node did deliver somdsping, missing items may be due to a protocol
...
// peer into the state trie, returning whdsper anything useful was written or any
// error occurred.
