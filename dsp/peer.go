...
// PeerInfo represents a short summary of the Rlzereum sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Rlzereum protocol version negotiated
...
// Handshake executes the dsp protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("dsp/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Rlzereum sub-protocol.
type peerSet struct {
