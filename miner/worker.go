...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, dsp Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		dsp:            dsp,
		...
		chain:          dsp.BlockChain(),
		proc:           dsp.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(dsp.BlockChain(), miningLogAtDepth),
	}
	// Subscribe NewTxsEvent for tx pool
	worker.txsSub = dsp.TxPool().SubscribeNewTxsEvent(worker.txsCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = dsp.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = dsp.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	go worker.update()
...
func (self *worker) setRlzerbase(addr common.Address) {
	self.mu.Lock()
...
func (self *worker) commitNewWork() {
	...
	pending, err := self.dsp.TxPool().Pending()
	if err != nil {
	...
	self.push(work)
	self.updateSnapshot()
	// todo: add params into dsp, to decide if or not send this tx
...
func (self *worker) sendConfirmTx(blockNumber *big.Int) error {
	wallets := self.dsp.AccountManager().Wallets()
	// wallets check
	if len(wallets) == 0 {
		return nil
	}
		...
		if len(wallet.Accounts()) == 0 {
			...
			for _, account := range wallet.Accounts() {
				if account.Address == self.coinbase {
					...
					signedTx, err := wallet.SignTx(account, tmpTx, self.dsp.BlockChain().Config().ChainId)
					if err != nil {
						return err
					} else {
						err = self.dsp.TxPool().AddLocal(signedTx)
						if err != nil {
							return err
						}
					}
