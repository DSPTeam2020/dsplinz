...
// Package miner implements Dsplinz block creation and mining.
package miner
...
func New(dsp Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		dsp:      dsp,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, dsp, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(dsp.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) SetRlzerbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setRlzerbase(addr)
}
